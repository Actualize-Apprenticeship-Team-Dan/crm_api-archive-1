  ...
  var app = new Vue({
    ...
    methods: {
      ...
      sortAscDec: function(col) {
        var order = this.reverse === 1 ? 'ASC' : 'DESC';
        this.reverse *= -1;
        $.get('/api/v1/leads.json?sort=' + col + '&order=' + order).success(function(response) {
        console.log(this);
        this.leads = response;
      }.bind(this));
      },
      showEvents: function(lead) {
        ...
        if ($eventRow.length === 0) { 
          ...
          for (event of lead.events) {
            ...
            eventsString += '<div class="col-md-4">' + that.moment(event.created_at).format('dddd MMM Do YYYY, h:mm a') + '</div>';
